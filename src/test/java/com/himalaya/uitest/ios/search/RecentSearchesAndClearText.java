...
public class RecentSearchesAndClearText extends AbstractTestCase {
    ...
    public void testRecentSearchesAndClearText(){
        ...
        //验证输入框中显示搜索内容
        Assert.assertTrue(iosDriver.findElement(searchPage.SEARCH_CONTENT_SEARCH_INPUT()).getAttribute("value").equals("testiosgui"),
                "验证输入框中显示搜索内容");
        ...
        //验证搜索过文字存在
        Assert.assertTrue(iosDriver.findElement(By.xpath("//XCUIElementTypeStaticText[@name='testiosgui']")).isDisplayed(),
                "验证搜索过的文字存在");
    }
}
